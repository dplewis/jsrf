...
def main():
    ap = argparse.ArgumentParser(description='Patches re-implementation EXE into original jsrf XBE')
    ...
    for de in pe.DIRECTORY_ENTRY_IMPORT:
        ...
        if image_name not in ('xboxkrnl.exe', 'jsrf.xbe'):
    ...
    print('xbe entry addr: %x', xbe.entry_addr)
    ...
    with open(root_dir + '/.gdbinit', 'w') as f:
        ...
        f.write('add-symbol-file build/jsrf ' + hex(exe_to_xbe_section_map['.text']) + ' ' +
    ...
    log.info('Verifying PATCHED XBE MD5')
    with open(args.output_xbe, 'rb') as f:
        md5 = hashlib.md5(f.read()).hexdigest()
        log.info('MD5: %s', md5)
